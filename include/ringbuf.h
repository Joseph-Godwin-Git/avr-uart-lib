 ...
 *
 * Template parameter BufferSize must be a power of two to enable optimized
 * index wrapping.
 *
...
{
 public:
  // Compile-time check: BufferSize must be a power of two for bitwise modulo
  // optimization
   ...
   */
  bool isEmpty(void) const { return (_writeIndex == _readIndex); }
  ...
  void flush(void) { _writeIndex = _readIndex; }
